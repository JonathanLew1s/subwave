...
export async function runAnalysisPass(opts: AnalyzeOptions = {}): Promise<AnalyzeStats> {
  ...
  //
  // Prefetch is eagerly settled to {path}|{err} so a rejection can never float
  // as an unhandled rejection in the window between kicking off the download and
  // awaiting it next iteration (Node's --unhandled-rejections=throw would crash
  // the pass on stale library entries otherwise).
  ...
  let inflight: Prefetch | null = usePrefetch && ids.length > 0 ? prefetch(ids[0]) : null;

  for (let i = 0; i < ids.length; i++) {
    const id = ids[i];
    const downloadPromise = inflight;
    // Kick off the NEXT download before awaiting this one's analysis so the
    // fetch overlaps the compute (local backend only — sidecar skips prefetch).
    inflight = usePrefetch && i + 1 < ids.length ? prefetch(ids[i + 1]) : null;
    ...
    try {
      if (usePrefetch) {
        ...
        if (settled && 'err' in settled) {
          ...
          // not retryable via the url path; let the per-track handler record it.
