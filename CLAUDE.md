...
- **Music Assistant backend** (`library.backend` = `ma-api`, the alternative to Navidrome) — `music/library.ts` is a facade that branches on `config.libraryBackend` per function; `music/library-ma.ts` + `music/ma-db-api.ts` are the MA-mode implementation, talking HTTP to a separate sidecar repo (`music-assistant-db-api`, a Rust service exposing MA's own SQLite library as a REST API — not part of this repo). Mood matching in MA mode (`songsByMood`/`songsByMoods`) is **energy-band-only**: a fixed `MOOD_ENERGY` table maps each of 17 mood labels to an `energy_min`/`energy_max` range, queried via the sidecar's `/tracks?energy_min=&energy_max=` — there is no genre/vocal-character discrimination at that layer, so a union of bands can admit wildly off-brief genres (confirmed live: trap and arena rock both landing in a "sunny indie pop" energy band). The sidecar also computes richer per-track axes (`valence`, `arousal`, `danceability`, `acousticness`, `instrumentalness`, `brightness`, `speechiness`, `roughness`, CLAP embeddings) and a metadata-derived `popularity` (almost always `null` — few providers populate it, but real values are starting to appear for some tracks; always treat as optional) — `library-ma.ts`'s `toLibraryTrack` surfaces all of these null-safely. **Audio-analysis coverage tracks an ongoing background tagger job — verify live before relying on a coverage number, don't trust this doc's.** Confirmed 2026-06-25: full coverage (`has_analysis=true`/`has_clap=true` both return the same total as no filter — see the reference doc below for the exact commands). A 2026-06-19 note here claimed "~20%, skewed alphabetically" and was repeated as current fact for nearly a week after the job had actually finished. Re-run the verification commands rather than editing in a new fixed percentage here. energy/mood/sonic-axis filtering can only ever surface analysed tracks regardless of coverage level (the sidecar query inner-joins against `audio_analysis`, not a left join) — that structural behaviour holds even at full coverage. `tracksLikeThis`/`tracksLikeThisAudio` route to the sidecar's real CLAP-vector KNN (`/tracks/:id/similar`) — genuine audio similarity, unlike Navidrome's old last.fm-based "similar songs." **`tracksByVector`/`tracksByAudioVector`/`recentPicksSimilarity` have no MA branch and fail closed (`[]`/`null`) in MA mode** rather than querying the Navidrome-only local SQLite vector table, which is never populated for MA-backed tracks — there's no sidecar endpoint for free-text/raw-vector search today, only id-anchored KNN.
- **MA composite shortlist** (`music/ma-candidate-pool.ts`, `music/theme-centroid.ts`) — a candidate shortlist (theme/flow/discovery/oldie) for MA-backed shows, **live on both the agent-pick and `auto.m3u` paths** for any show with a usable exemplar profile. Gates on a show's `exemplarTrackIds` (0-8 real library tracks) via their literal genre tags (a palette, hard filter) plus CLAP-vector similarity ranking within that palette — replaced an earlier 6-axis sonic centroid approach, confirmed live to still admit cross-genre noise (Ella Fitzgerald, Bee Gees, Dean Martin) because averaging genre-diverse exemplars on continuous sonic axes washes out the discriminating signal a literal genre tag doesn't. Full mechanism, the trade-offs found applying this gate to similarity-search tools specifically, and a verified field-by-field liveness table for show config (which fields are dead on which pick path) are in `docs/superpowers/specs/2026-06-25-picker-pipeline-reference.md` — read that before changing picker behaviour rather than re-deriving it from this paragraph. Shadow comparisons (`broadcast/picker-shadow-log.ts`) still exist for shows WITHOUT a usable exemplar profile (showing what adding exemplars would change) via `dj-agent.ts`'s `maybeRunPickerShadow` and `scheduler.ts`'s `maybeRunAutoPlaylistShadow`, gated on `picker.maShortlist.shadowEnabled`/`autoPlaylistShadowEnabled`. `GET /dj/picker-shadow?kind=live-pick|auto-playlist` (admin-gated) inspects the log.
