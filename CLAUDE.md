...
- **Music Assistant backend** (`library.backend` = `ma-api`, the alternative to Navidrome) — `music/library.ts` is a facade that branches on `config.libraryBackend` per function; `music/library-ma.ts` + `music/ma-db-api.ts` are the MA-mode implementation, talking HTTP to a separate sidecar repo (`music-assistant-db-api`, a Rust service exposing MA's own SQLite library as a REST API — not part of this repo). Mood matching in MA mode (`songsByMood`/`songsByMoods`) is **energy-band-only**: a fixed `MOOD_ENERGY` table maps each of 17 mood labels to an `energy_min`/`energy_max` range, queried via the sidecar's `/tracks?energy_min=&energy_max=` — there is no genre/vocal-character discrimination at that layer, so a union of bands can admit wildly off-brief genres (confirmed live: trap and arena rock both landing in a "sunny indie pop" energy band). The sidecar also computes richer per-track axes (`valence`, `arousal`, `danceability`, `acousticness`, `instrumentalness`, `brightness`, `speechiness`, `roughness`, CLAP embeddings) and a metadata-derived `popularity` (almost always `null` — few providers populate it, but real values are starting to appear for some tracks; always treat as optional) — `library-ma.ts`'s `toLibraryTrack` surfaces all of these null-safely. **Audio-analysis coverage is partial and currently skewed alphabetically** (at last check, ~20% of the library analysed, ~90% of *that* slice being artists A–C, because MA's analysis pass works roughly in scan order) — energy/mood/sonic-axis filtering can only ever surface analysed tracks (the underlying sidecar query inner-joins against `audio_analysis`, not a left join), so this skew is structural, not a query bug, and self-corrects only as MA's own analysis progresses. `tracksLikeThis`/`tracksLikeThisAudio` route to the sidecar's real CLAP-vector KNN (`/tracks/:id/similar`) — genuine audio similarity, unlike Navidrome's old last.fm-based "similar songs." **`tracksByVector`/`tracksByAudioVector`/`recentPicksSimilarity` have no MA branch and fail closed (`[]`/`null`) in MA mode** rather than querying the Navidrome-only local SQLite vector table, which is never populated for MA-backed tracks — there's no sidecar endpoint for free-text/raw-vector search today, only id-anchored KNN.
- **MA composite shortlist** (`music/ma-candidate-pool.ts`, `music/theme-centroid.ts`) — a 4-slot candidate shortlist (theme/flow/discovery/oldie) for MA-backed shows, built to go beyond the flat energy-band matching above. **Shadow-only as of writing — not wired into any live pick or `auto.m3u`.** `buildMaShortlist()` composes: theme (mood-band pool, optionally gated by `theme-centroid.ts`'s `gateByCentroid` — see below), flow (CLAP similarity + bpm/key compat to the currently-playing track, via `tracksLikeThis`), discovery (same mood pool, biased toward artists not already used elsewhere in the shortlist, to stop a flow-anchored chain drifting into a sonic rut), and oldie (era-filtered, ranked by the still-mostly-null `popularity` then `favorite`, omitted entirely rather than forced if nothing qualifies). `theme-centroid.ts` turns a show's optional `exemplarTrackIds` (settings field, 0-8 ids, empty by default — fully backward compatible) into a centroid in the 6-axis sonic space, then **gates** (admits into an auto-sized pool) rather than **ranks** mood-band candidates by distance to it — validated live that ranking by raw closeness would replay the same handful of tracks across a multi-hour show, where gating into a large-enough pool (auto-sized via `themeCentroid.minPoolSize`, capped at `maxThreshold`) lets the existing recency/randomization machinery actually rotate through variety. An exemplar with no analysis coverage is silently excluded from the centroid average (not from the whole computation) — `computeThemeCentroid` returns `null`, meaning "fall back to flat behaviour," only when fewer than `themeCentroid.minExemplars` exemplars survive. Shadow comparisons (`broadcast/picker-shadow-log.ts`, a ring-buffer+JSONL log modelled on `request-log.ts`) run fire-and-forget off two hooks — `dj-agent.ts`'s `maybeRunPickerShadow` (after every live pick, gated on `picker.maShortlist.shadowEnabled`) and `scheduler.ts`'s `maybeRunAutoPlaylistShadow` (after every `auto.m3u` refresh, gated on the separate `picker.maShortlist.autoPlaylistShadowEnabled`) — both default off, both log what the gated shortlist *would* have produced without ever touching what actually plays. `GET /dj/picker-shadow?kind=live-pick|auto-playlist` (admin-gated) inspects the log.
...
- **MA mode**: every function in `library.ts` that branches on backend must have an explicit `ma-api` arm — three (`tracksByVector`, `tracksByAudioVector`, `recentPicksSimilarity`) were missed when MA support was added and silently queried the Navidrome-only local SQLite vector table, which is never populated for MA-backed tracks; they now fail closed instead. When fetching a single MA track for its analysis (bpm/key/sonic axes), the sidecar requires `?include=analysis` explicitly — omitting it silently returns every analysis field as `null` regardless of real coverage (this is what made the flow slot's bpm/key-compat scoring a no-op until fixed). Don't assume `order=random` results are evenly spread across the catalogue — the underlying analysed slice is itself skewed (see Music Assistant backend above); a perfectly random sample of a 90%-A/B/C pool still looks alphabetically front-loaded.
